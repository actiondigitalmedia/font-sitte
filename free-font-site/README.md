...
### Browse the catalog (UI) + SEO static site

Python + **Jinja2** builds indexable specimen/hub pages (no Astro/Eleventy required — see `seo/STANDARDS.md`).

```bash
cd free-font-site
pip install -r requirements.txt
python3 scripts/build_web_catalog.py   # if catalog-lite needs refresh
python3 scripts/build_site.py          # → dist/ (fonts, hubs, sitemaps, schema)
python3 scripts/verify_seo.py
python3 scripts/serve.py 8080
# open http://localhost:8080/fonts/roboto/
# full pipeline: python3 scripts/build_all.py
```

SEO surface area: `/fonts/{id}/`, `/category/`, `/source/`, `/licenses/`, `/tags/`, `/use/`, `/commercial-use/`, sitemap index, JSON-LD (entity + breadcrumbs + FAQ).
