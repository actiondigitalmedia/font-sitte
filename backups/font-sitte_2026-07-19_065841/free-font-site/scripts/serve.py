...
DIST = ROOT / "dist"
SERVE_ROOT = DIST if (DIST / "index.html").exists() else ROOT
ALLOWED_PREFIXES = (
    "https://fonts.gstatic.com/",
    "https://cdn.fontshare.com/",
    "https://cdn.jsdelivr.net/fontsource/",
)


class FontSiteHandler(SimpleHTTPRequestHandler):
    def __init__(self, *args, directory: str | None = None, **kwargs):
        super().__init__(*args, directory=directory or str(SERVE_ROOT), **kwargs)
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/proxy-font":
            ...
            return
        if parsed.path == "/" and SERVE_ROOT == ROOT:
            self.send_response(302)
            self.send_header("Location", "/web/")
            self.end_headers()
            return
        if parsed.path == "/web/" and SERVE_ROOT == DIST:
            self.send_response(302)
...
def main() -> None:
    ...
    handler = partial(FontSiteHandler, directory=str(SERVE_ROOT))
    server = ThreadingHTTPServer(("0.0.0.0", port), handler)
    print(f"Serving {SERVE_ROOT} on http://0.0.0.0:{port}")
    ui = "/" if SERVE_ROOT == DIST else "/web/"
